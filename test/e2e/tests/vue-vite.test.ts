...
import { configureInstalledMcpVueVite } from "../helpers/installer.js";
...
describe("Vue + Vite E2E", () => {
  ...
  beforeAll(async () => {
    configureInstalledMcpVueVite([MCP_ORIGIN, `http://localhost:${PORT}`], MCP_TOKEN);
  ...
  it("keeps browser operations usable after MCP failure and ACP reconnect", async () => {
    ...
    const marker = await proveRecoveryAfterFrameworkMcpFailure(page, context, PORT, "src/App.vue");
