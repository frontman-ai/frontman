...
describe("Astro E2E", () => {
	...
	itWithOpenAI(
		"should sync the preview URL after client-side navigation",
		async () => {
			page = await context.newPage();
			await openFrontmanUI(page, PORT);

			const preview = page.frameLocator(
				`iframe[title^="Preview -"][src="http://localhost:${PORT}/"]`,
			);
	...
	itWithOpenAI("should make a text change via AI prompt", async () => {
		...
		await sendPrompt(
			page,
			'Change the h1 heading text in src/pages/index.astro to say "Hello Frontman"',
		);
