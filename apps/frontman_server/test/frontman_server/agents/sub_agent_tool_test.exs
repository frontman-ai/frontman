defmodule FrontmanServer.Agents.SubAgentToolTest do
  ...
  describe "tool_definition/0" do
    test "returns valid tool definition" do
      ...
      assert "task" in definition.parameters.required
    end
  ...
  describe "parse_arguments/1" do
    test "parses valid research agent arguments" do
      args = %{"agent" => "research", "task" => "Find information about X"}
      assert {:ok, parsed} = SubAgentTool.parse_arguments(args)
      assert parsed.role == :research
      assert parsed.task == "Find information about X"
    end

    test "parses valid planning agent arguments" do
      args = %{"agent" => "planning", "task" => "Create a plan for Y"}
      assert {:ok, parsed} = SubAgentTool.parse_arguments(args)
      assert parsed.role == :planning
      assert parsed.task == "Create a plan for Y"
    end

    test "parses valid validator agent arguments" do
      args = %{"agent" => "validator", "task" => "Validate the implementation"}
      assert {:ok, parsed} = SubAgentTool.parse_arguments(args)
      assert parsed.role == :validator
      assert parsed.task == "Validate the implementation"
    end

    test "returns error for invalid agent type" do
      args = %{"agent" => "invalid_type", "task" => "Do something"}
    ...
    test "returns error for empty task" do
      args = %{"agent" => "research", "task" => "   "}
      assert {:error, msg} = SubAgentTool.parse_arguments(args)
      assert msg =~ "empty"
    end
    ...
    test "returns error for missing task parameter" do
      args = %{"agent" => "research"}
      assert {:error, msg} = SubAgentTool.parse_arguments(args)
      assert msg =~ "task"
    end
