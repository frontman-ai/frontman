...
defmodule FrontmanServerWeb.ErrorHTML do
  ...
  def render(template, assigns) do
    ...
    <main class="min-h-screen bg-slate-950 px-6 py-16 text-white">
      <section class={[
        "mx-auto flex max-w-2xl flex-col gap-8 rounded-3xl border border-white/10",
        "bg-white/[0.04] p-8 shadow-2xl shadow-black/30 sm:p-10"
      ]}>
        <div class="space-y-3">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <a
            class={[
              "rounded-2xl bg-primary px-5 py-4 text-center text-sm font-semibold",
              "text-primary-content transition hover:brightness-110"
            ]}
          ...
          <a
            class={[
              "rounded-2xl border border-white/15 px-5 py-4 text-center text-sm font-semibold",
              "text-white transition hover:border-primary/70 hover:text-primary"
            ]}
            href={@discord_url}
            target="_blank"
        ...
        <p class="text-sm leading-6 text-slate-400">
          ...
          error. Support email:
          <a class="text-primary underline underline-offset-4" href={"mailto:#{@support_email}"}>
            {@support_email}
          </a>
        </p>
      </section>
