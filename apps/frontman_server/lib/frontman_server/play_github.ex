...
defmodule FrontmanServer.PlayGithub do
  ...
  alias FrontmanServer.PlayGithub.{Daytona, GithubReference}
  alias FrontmanServer.PlayGithub.GithubReference.{RepositoryPath, TreePath}
  ...
  def run_repository_command(
        %GithubReference{resource: %RepositoryPath{}} = github_reference,
        command,
        opts
      )
      when command in [:create, :start, :clone, :install, :dev] do
    run_repository_target_command(repository_target(github_reference), command, opts)
  end

  def run_repository_command(
        %GithubReference{resource: %TreePath{}} = github_reference,
        command,
        opts
      )
      when command in [:create, :start, :clone, :install, :dev] do
  ...
  def run_repository_command(%GithubReference{}, command, _opts)
      when command in [:create, :start, :clone, :install, :dev] do
    {:error, :not_repository_path}
  end
  ...
  defp repository_target(%GithubReference{} = github_reference) do
    repo_url = GithubReference.github_url(github_reference)
    ...
    |> Map.merge(%{
      repo_url: repo_url,
      sandbox_name:
        sandbox_name_for_identity(GithubReference.repository_identity(github_reference))
    })
  end

  defp resource_target(%GithubReference{resource: %RepositoryPath{}}) do
    %{github_ref: nil, repo_path: nil, workspace_path: "workspace"}
  end

  defp resource_target(%GithubReference{resource: %TreePath{} = tree_path}) do
    repo_path = TreePath.repo_path(tree_path)
