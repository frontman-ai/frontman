...
describe("analytics consent", () => {
  ...
  test("tracks a completed task demonstration once per page load", () => {
    const {dom} = createPage({analytics: true})
    pages.push(dom)

    const video = dom.window.document.createElement("video")
    video.dataset.gaEvent = "wordpress_task_demo_completed"
    video.dataset.gaTrigger = "ended"
    video.dataset.gaCategory = "engagement"
    video.dataset.gaLabel = "faq_title"
    video.dataset.gaPlacement = "embedded_demo"
    video.dataset.gaTaskFamily = "copy_update"
    ...
    expect(events).toEqual([
      [
        "event",
        "wordpress_task_demo_completed",
        {
          event_category: "engagement",
          event_label: "faq_title",
          page_path: "/",
          placement: "embedded_demo",
          task_family: "copy_update",
        },
      ],
    ])
  })

  test("does not track a completed task demonstration without consent", () => {
    const {dom} = createPage()
    pages.push(dom)
