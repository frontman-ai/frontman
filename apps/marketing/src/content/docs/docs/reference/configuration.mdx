...
- In Next.js, make sure your `matcher` also covers the custom path if you wire middleware/proxy manually.
...
Use the Next.js integration in `middleware.ts` for Next.js 15.x (15.5 minimum) or `proxy.ts` for Next.js 16.x.

Place the file next to your `app/` or `pages/` directory. For projects using `src/app` or `src/pages`, that means `src/middleware.ts` or `src/proxy.ts`.
