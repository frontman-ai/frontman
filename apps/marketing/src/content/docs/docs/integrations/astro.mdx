...
| `mcp` | `{ allowedOrigins, authorize }` | Disabled | Explicit Origin allowlist and authorization callback required to expose exact `/mcp`. |
...
**`frontman-middleware`** — Installs a Connect middleware that serves the `/{basePath}/*` UI and source-location routes. With explicit `mcp` security configuration, it separately handles exact `POST /mcp` for `server/discover`, `tools/list`, and `tools/call`. The endpoint returns JSON today; the browser client also accepts standard MCP SSE responses. Deprecated private tool-call and GET event-stream routes are not exposed, and `/mcp` never uses wildcard CORS.
