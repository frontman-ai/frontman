...
`@frontman-ai/nextjs` integrates the Frontman AI agent into your Next.js dev server through Next.js middleware on Next.js 15.5+ or proxy on Next.js 16+. It intercepts requests to `/frontman/*`, giving the AI access to your source files, dev server logs, route manifest, and optionally OpenTelemetry spans covering HTTP requests and route render timing.
...
Run `npx @frontman-ai/nextjs install` from your project directory, start `next dev`, then open `http://localhost:3000/frontman`. For Next.js 16, the installer writes `proxy.ts`; for Next.js 15.5+, it writes `middleware.ts` with a matcher scoped to Frontman routes. If your app uses `src/app` or `src/pages`, the installer writes the entrypoint under `src/`.
...
- Node.js 18 or later
- Next.js 15.5 or later (`^15.5.0 || ^16.0.0`)
...
The CLI detects your Next.js version and writes the appropriate middleware/proxy file. To install manually without the CLI:
...
When the request does not match a Frontman path, `frontman(req)` returns `undefined` (Next.js 15.5+) or a pass-through `NextResponse.next()` (Next.js 16+).
...
Verify the `matcher` in your `middleware.ts` or `proxy.ts` `config` export includes `/frontman` and `/frontman/:path*`. Without the matcher, Next.js does not route those requests through Frontman at all.
