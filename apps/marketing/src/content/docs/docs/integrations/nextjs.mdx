...
Run `npx @frontman-ai/nextjs install` from your project directory, start `next dev`, then open `http://localhost:3000/frontman`. For Next.js 16.x, the installer writes `proxy.ts`; for Next.js 15.x (15.5 minimum), it writes `middleware.ts` with a matcher scoped to Frontman routes. If your app uses `src/app` or `src/pages`, the installer writes the entrypoint under `src/`.
...
- Node.js 18 or later
- Next.js 15.x (15.5 minimum) or 16.x (`^15.5.0 || ^16.0.0`)
...
`createMiddleware` returns an async function compatible with the Next.js middleware API and the Next.js 16.x proxy API. When a request arrives at `/frontman/*`:
...
Ensure `instrumentation.ts` is in the project root or `src/` and that the `NEXT_RUNTIME === 'nodejs'` guard is present.
