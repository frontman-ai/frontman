...
`@frontman-ai/nextjs` integrates the Frontman AI agent into your Next.js dev server through Next.js middleware on Next.js 15 or proxy on Next.js 16+. It intercepts requests to `/frontman/*`, giving the AI access to your source files, dev server logs, route manifest, and optionally OpenTelemetry spans covering HTTP requests and route render timing.
...
When the request does not match a Frontman path, `frontman(req)` returns `undefined` (Next.js 15) or a pass-through `NextResponse.next()` (Next.js 16+).
