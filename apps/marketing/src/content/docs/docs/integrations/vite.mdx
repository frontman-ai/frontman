...
**Main middleware plugin** — Registers a Connect middleware via the `configureServer` hook. It serves the `/{basePath}/*` UI and source-location routes and, with explicit `mcp` security configuration, separately handles exact `POST /mcp` for `server/discover`, `tools/list`, and `tools/call`. The endpoint returns JSON today; the browser client also accepts standard MCP SSE responses. Deprecated private tool-call and GET event-stream routes are not exposed. The middleware bridges Vite's Node.js request/response API to the internal Web Fetch API.
