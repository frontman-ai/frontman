...
@get external title: WebAPI.DomTypes.document => string = "title"
@get
external matchMediaMethod: WebAPI.DomTypes.window => Nullable.t<
  string => WebAPI.DomTypes.mediaQueryList,
> = "matchMedia"

@get external matches: WebAPI.DomTypes.mediaQueryList => bool = "matches"

let read = (): Preview.pageContext => {
  let window = WebAPI.Window.current
  let colorScheme = switch window->matchMediaMethod {
  | Value(_) =>
