...
describe("LogCapture", _t => {
  describe("Initialization", _t => {
    test("getInstance creates singleton on first call", t => {
    ...
    test("initialize is idempotent", t => {
      LogCapture.initialize()
      LogCapture.initialize()
      LogCapture.initialize()
  ...
  describe("Console Interception", _t => {
    test("console.log captures message", t => {
      ...
      let found = logs->Array.some(log =>
        log.message->String.includes("test message unique 123")
      )

      t->expect(found)->Expect.toBe(true)
    })

    test("console.warn captures message", t => {
      Console.warn("warning message unique 456")

      let logs = LogCapture.getLogs()
      let found = logs->Array.some(log =>
        log.message->String.includes("warning message unique 456")
      )

      t->expect(found)->Expect.toBe(true)
    })

    test("console.error captures message", t => {
      Console.error("error message unique 789")

      let logs = LogCapture.getLogs()
      let found = logs->Array.some(log =>
        log.message->String.includes("error message unique 789")
      )

      t->expect(found)->Expect.toBe(true)
    })

    test("multiple arguments converted to space-separated string", t => {
  ...
  describe("ANSI Stripping", _t => {
    test("ANSI color codes are stripped before storage", t => {
    ...
    test("multiple ANSI codes are stripped", t => {
  ...
  describe("Query Functions - getLogs", _t => {
    describe("Pattern Matching", _t => {
      test("no pattern returns all logs", t => {
        Console.log("first")
        Console.log("second")
        Console.log("third")
      ...
      test("pattern filter returns matching entries", t => {
      ...
      test("pattern matching is case-insensitive", t => {
      ...
      test("regex pattern works correctly", t => {
    ...
    describe("Level Filtering", _t => {
      test("level filter returns only matching level", t => {
    ...
    describe("Tail Limiting", _t => {
      test("tail limits returned results", t => {
      ...
      test("tail returns most recent logs", t => {
        Console.log("oldest")
        Console.log("middle")
        Console.log("newest")
    ...
    describe("Timestamp Filtering", _t => {
      test("since filter returns only recent logs", t => {
        ...
        if allLogs->Array.length > 0 {
          ...
          let allRecent = recentLogs->Array.every(log => {
  ...
  describe("Metadata", _t => {
    test("timestamp is in ISO 8601 format", t => {
      Console.log("test")

      let logs = LogCapture.getLogs()
      let hasISOTimestamp = logs->Array.some(log => {
    ...
    test("timestamps are chronologically ordered", t => {
      Console.log("first")
      Console.log("second")
      Console.log("third")

      let logs = LogCapture.getLogs()
      let testLogs = logs->Array.filter(log =>
        log.message == "first" || log.message == "second" || log.message == "third"
      )
  ...
  describe("Buffer Integration", _t => {
    test("logs are stored in buffer correctly", t => {
    ...
    test("buffer enforces capacity limit", t => {
  ...
  describe("Error Handling", _t => {
    test("invalid regex returns empty array silently", t => {
    ...
    test("log capture errors don't crash app", t => {
