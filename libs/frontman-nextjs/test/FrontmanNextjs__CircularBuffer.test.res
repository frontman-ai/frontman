...
describe("CircularBuffer", _t => {
  describe("Basic Operations", _t => {
    test("make creates empty buffer with correct capacity", t => {
      let buffer = CircularBuffer.make(~capacity=10)
      t->expect(CircularBuffer.length(buffer))->Expect.toBe(0)
    })

    test("length returns 0 for new buffer", t => {
      let buffer = CircularBuffer.make(~capacity=5)
      t->expect(CircularBuffer.length(buffer))->Expect.toBe(0)
    })

    test("toArray returns empty array for new buffer", t => {
      let buffer = CircularBuffer.make(~capacity=5)
  ...
  describe("Push and Capacity", _t => {
    test("push single item increases length to 1", t => {
      let buffer = CircularBuffer.make(~capacity=10)
    ...
    test("push N items (N < capacity) returns all in order", t => {
      let buffer = CircularBuffer.make(~capacity=10)
      let buffer = CircularBuffer.push(buffer, "a")
    ...
    test("push exactly capacity items returns all items", t => {
    ...
    test("push capacity + 1 items evicts oldest", t => {
      let buffer = CircularBuffer.make(~capacity=3)
      let buffer = CircularBuffer.push(buffer, "a")
      let buffer = CircularBuffer.push(buffer, "b")
      let buffer = CircularBuffer.push(buffer, "c")
    ...
    test("push 2x capacity items keeps only last capacity items", t => {
      let buffer = CircularBuffer.make(~capacity=3)
      let buffer = CircularBuffer.push(buffer, "a")
      let buffer = CircularBuffer.push(buffer, "b")
      let buffer = CircularBuffer.push(buffer, "c")
  ...
  describe("Chronological Ordering", _t => {
    test("items not wrapped return in insertion order", t => {
      let buffer = CircularBuffer.make(~capacity=10)
    ...
    test("items wrapped once return in correct chronological order", t => {
      let buffer = CircularBuffer.make(~capacity=3)
      let buffer = CircularBuffer.push(buffer, "a")
      let buffer = CircularBuffer.push(buffer, "b")
      let buffer = CircularBuffer.push(buffer, "c")
    ...
    test("items wrapped multiple times maintain order", t => {
      let buffer = CircularBuffer.make(~capacity=3)
  ...
  describe("Edge Cases", _t => {
    test("capacity 1 buffer keeps only most recent item", t => {
    ...
    test("clear buffer resets to empty state", t => {
      let buffer = CircularBuffer.make(~capacity=5)
      let buffer = CircularBuffer.push(buffer, "a")
      let buffer = CircularBuffer.push(buffer, "b")
      let buffer = CircularBuffer.push(buffer, "c")
    ...
    test("push after clear works correctly", t => {
      let buffer = CircularBuffer.make(~capacity=3)
      let buffer = CircularBuffer.push(buffer, "a")
      let buffer = CircularBuffer.push(buffer, "b")
      let buffer = CircularBuffer.clear(buffer)
    ...
    test("very large capacity works without issues", t => {
  ...
  describe("Immutability", _t => {
    test("push returns new buffer state", t => {
    ...
    test("multiple pushes create proper state progression", t => {
      let buffer = CircularBuffer.make(~capacity=5)
