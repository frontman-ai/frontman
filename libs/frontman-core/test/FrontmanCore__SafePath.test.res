...
describe("resolve", () => {
  test(
    "resolves relative path under sourceRoot",
    t => {
      let result = SafePath.resolve(~sourceRoot="/project", ~inputPath="src/file.ts")
      switch result {
      | Ok(safePath) =>
        t->expect(SafePath.toString(safePath))->Expect.toBe("/project/src/file.ts")
  ...
  test(
    "resolves dot path to sourceRoot",
    t => {
      let result = SafePath.resolve(~sourceRoot="/project", ~inputPath=".")
      switch result {
      | Ok(safePath) =>
        t->expect(SafePath.toString(safePath))->Expect.toBe("/project")
      | Error(msg) => t->expect(msg)->Expect.toBe("should not fail")
      }
    },
  )
  ...
  test(
    "rejects path escaping sourceRoot via ..",
    t => {
  ...
  test(
    "accepts absolute path under sourceRoot",
    t => {
  ...
  test(
    "rejects absolute path outside sourceRoot",
    t => {
      let result = SafePath.resolve(~sourceRoot="/project", ~inputPath="/etc/passwd")
      switch result {
      | Ok(_) => t->expect("should have failed")->Expect.toBe("")
  ...
  test(
    "handles sourceRoot without trailing separator",
    t => {
  ...
  test(
    "handles sourceRoot with trailing forward slash",
    t => {
      let result = SafePath.resolve(~sourceRoot="/project/src/", ~inputPath="file.ts")
      switch result {
      | Ok(safePath) =>
  ...
  test(
    "path.sep is a valid separator character",
    t => {
...
describe("resolve - separator handling", () => {
  test(
    "sourceRoot at path boundary is enforced (no prefix collision)",
    t => {
      // /project-extra should NOT be accepted under /project
      let result = SafePath.resolve(
        ~sourceRoot="/project",
        ~inputPath="/project-extra/file.ts",
      )
      switch result {
  ...
  test(
    "accepts sourceRoot itself as absolute input",
    t => {
  ...
  test(
    "nested sourceRoot with similar prefix is safe",
    t => {
...
describe("dirname", () => {
  test(
    ...
    t => {
      ...
      | Ok(safePath) =>
        t->expect(SafePath.dirname(safePath))->Expect.toBe("/project/src")
      | Error(msg) => t->expect(msg)->Expect.toBe("should not fail")
      }
    },
  )
...
describe("join", () => {
  ...
  test(
    "rejects join that escapes sourceRoot",
    t => {
      switch SafePath.resolve(~sourceRoot="/project", ~inputPath="src") {
      | Ok(safePath) =>
