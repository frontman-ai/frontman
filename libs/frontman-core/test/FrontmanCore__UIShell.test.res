...
describe("UIShell", _t => {
  describe("generateHTML", _t => {
    ...
    test(
      "applies dark class when isLightTheme is false",
      t => {
    ...
    test(
      "omits dark class when isLightTheme is true",
      t => {
    ...
    test(
      "includes randomUUID polyfill for insecure origins",
      t => {
        let html = UIShell.generateHTML(Helpers.makeConfig())

        t
        ->expect(html->String.includes("typeof c.randomUUID===\"function\""))
        ->Expect.toBe(true)
        t
        ->expect(html->String.includes("Object.defineProperty(c,\"randomUUID\""))
        ->Expect.toBe(true)
      },
    )
  })
