...
describe("SearchFiles Tool - execute (integration)", _t => {
  ...
  testAsync(
    "should reject absolute search paths outside dot sourceRoot by falling back safely",
    async t => {
      let tempDir = await createTestFixture()
      ...
      try {
        ...
        let result = await SearchFiles.execute(
          ctx,
          {
            pattern: "Button.tsx",
            path: ?Some(Path.join([tempDir, "src"])),
          },
        )
        ...
        | Ok(output) =>
          t
          ->expect(output.files->Array.some(file => file->String.startsWith(tempDir)))
          ->Expect.toBe(false)
        | Error(msg) => failwith(`SearchFiles should handle dot roots: ${msg}`)
        }
      } catch {
      | exn => {
          let msg =
