// MCP Protocol Types

S.enableJson()

// Protocol version constant
let protocolVersion = "DRAFT-2025-v3"

// Tool definitions are serialized directly from modules - no intermediate type

// Capabilities
@schema
type capabilities = {
  tools: option<Dict.t<JSON.t>>,
  resources: option<Dict.t<JSON.t>>,
  prompts: option<Dict.t<JSON.t>>,
}

// Client/Server info
@schema
type info = {
  name: string,
  version: string,
}

// Initialize params (sent by client/agent)
@schema
type initializeParams = {
  protocolVersion: string,
  capabilities: capabilities,
  clientInfo: info,
}

// Initialize result (sent by server/browser)
@schema
type initializeResult = {
  protocolVersion: string,
  capabilities: capabilities,
  serverInfo: info,
}

// Tool call params
@schema
type toolCallParams = {
  callId: string,
  name: string,
  arguments: option<Dict.t<JSON.t>>,
}

// Tool result content
@schema
type toolResultContent = {
  @as("type") type_: string,
  text: string,
}

// Tool error
@schema
type toolError = {
  code: int,
  message: string,
}

// Tool result wrapper
@schema
type toolResult = {content: array<toolResultContent>}

// Tool call result
@schema
type toolCallResult = {
  callId: string,
  result: option<toolResult>,
  error: option<toolError>,
}

// Tools list result - tools serialized directly as JSON from modules
@schema
type toolsListResult = {tools: array<JSON.t>}

// MCP Error codes
module ErrorCode = {
  let invalidParams = -32602
  let serverError = -32000
  let methodNotFound = -32601
}
