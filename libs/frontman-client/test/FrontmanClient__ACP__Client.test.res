...
describe("ACP Client State Reducer", _t => {
  ...
  test("ConnectionStateChanged action updates connectionState", t => {
    ...
    let initResult: Types.initializeResult = {
      ...
      agentInfo: Some({name: "test", version: "1.0", title: None}),
      authMethods: None,
    }

    let newState = state->Client.reduce(
      Client.ConnectionStateChanged(Client.Initialized(initResult)),
    )
...
describe("ACP Client buildInitializeParams", _t => {
  test("builds correct JSON structure", t => {
    ...
    let config: Client.config = {
      channel: mockChannel,
      clientInfo: {name: "test-client", version: "1.0.0", title: None},
      clientCapabilities: {
