...
describe("JsonRpc Request", _t => {
  test("make creates request with correct fields", t => {
    let req = JsonRpc.Request.make(~id=JsonRpc.Id.fromInt(1), ~method="test", ~params=None)

    t->expect(req->JsonRpc.Request.id->JsonRpc.Id.toJson)->Expect.toEqual(JSON.Encode.int(1))
  ...
  test("toJson produces valid JSON-RPC 2.0 structure", t => {
    let req = JsonRpc.Request.make(~id=JsonRpc.Id.fromInt(42), ~method="initialize", ~params=None)
  ...
  test("toJson includes params when provided", t => {
    ...
    let req = JsonRpc.Request.make(~id=JsonRpc.Id.fromInt(1), ~method="test", ~params=Some(params))
    let json = req->JsonRpc.Request.toJson
    let obj = json->JSON.Decode.object->Option.getOrThrow
...
describe("JsonRpc Response", _t => {
  test("makeSuccess creates successful response", t => {
    ...
    let resp = JsonRpc.Response.makeSuccess(~id=JsonRpc.Id.fromInt(1), ~result)

    t->expect(resp->JsonRpc.Response.id->JsonRpc.Id.toJson)->Expect.toEqual(JSON.Encode.int(1))
  ...
  test("makeError creates error response", t => {
    ...
    let resp = JsonRpc.Response.makeError(~id=JsonRpc.Id.fromInt(2), ~error)

    t->expect(resp->JsonRpc.Response.id->JsonRpc.Id.toJson)->Expect.toEqual(JSON.Encode.int(2))
  ...
  test("fromJsonExn parses valid response", t => {
    ...
    t->expect(resp->JsonRpc.Response.id->JsonRpc.Id.toJson)->Expect.toEqual(JSON.Encode.int(99))
    t->expect(resp->JsonRpc.Response.isSuccess)->Expect.toEqual(true)
  })
