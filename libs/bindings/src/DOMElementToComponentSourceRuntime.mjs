...
export async function resolveSourceLocationInServer(sourceLocation, projectRoot) {
  ...
  const mapFile = existsSync(adjacentMap)
    ...
    : existsSync(alternateMap)
      ? alternateMap
      : undefined
  if (!mapFile) throw new Error(`Source map not found for ${generatedFile}`)
  ...
  if (original.source == null || original.line == null) {
    throw new Error(
      `Source map has no original position for ${generatedFile}:${sourceLocation.line}:${sourceLocation.column}`,
    )
  }
