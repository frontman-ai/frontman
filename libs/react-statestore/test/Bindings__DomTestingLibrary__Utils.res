...
module WaitFor = {
  type options = {
    ...
    "onTimeout": Js.undefined<JsExn.t => JsExn.t>,
    "showOriginalStackTrace": Js.undefined<bool>,
    "stackTraceError": Js.undefined<JsExn.t>,
  ...
  external makeOptions: (
    ...
    ~onTimeout: JsExn.t => JsExn.t=?,
    ~showOriginalStackTrace: bool=?,
    ~stackTraceError: JsExn.t=?,
    ~timeout: int=?,
    unit,
  ) => options = ""
}
...
external _waitFor: (unit => unit, Js.undefined<WaitFor.options>) => promise<'a> = "waitFor"

let waitFor = (~callback, ~options=?, ()) => _waitFor(callback, Js.Undefined.fromOption(options))

@module("@testing-library/dom")
external _waitForPromise: (unit => promise<'a>, Js.undefined<WaitFor.options>) => promise<'b> =
  "waitFor"
...
) => promise<unit> = "waitForElementToBeRemoved"
...
module Configure = {
  type options = {
    ...
    "getElementError": Js.undefined<(string, Dom.element) => JsExn.t>,
    "showOriginalStackTrace": Js.undefined<bool>,
  ...
  external makeOptions: (
    ...
    ~getElementError: (string, Dom.element) => JsExn.t=?,
    ~showOriginalStackTrace: bool=?,
...
@module("@testing-library/dom")
external configureWithFn: ({..} => {..}) => unit = "configure"
