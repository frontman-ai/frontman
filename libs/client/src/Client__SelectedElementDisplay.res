  ...
  let navigateUp = () => {
    ...
    | Some({sourceLocation: Some(currentLoc), element, selector, screenshot}) => {
        ...
        | Some(parentLoc) => {
            ...
            let parentElement = element->WebAPI.Element.asNode->WebAPI.Node.parentElement->Null.toOption
    ...
    if historyLength > 0 {
      ...
      | Some(_) => {
          ...
          | Some(previousState) => {
              ...
              let newSelectedElement: option<Client__State__StateReducer.SelectedElement.t> = Some(previousState)
