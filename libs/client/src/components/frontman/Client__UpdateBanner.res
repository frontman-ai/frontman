 ...
 *
 * Reads the discovered framework server identity, maps the
...
let make = () => {
  ...
  let {frameworkServerInfo, session, createSession} = Client__FrontmanProvider.useFrontman()

  React.useEffect3(() => {
    switch (updateCheckStatus, frameworkServerInfo, hasActiveACPSession) {
    | (UpdateNotChecked, Some(serverInfo), true) =>
    ...
    None
  }, (updateCheckStatus, frameworkServerInfo, hasActiveACPSession))
