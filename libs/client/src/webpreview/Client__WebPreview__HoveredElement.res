...
let make = (~element: option<Null.t<WebAPI.EventAPI.eventTarget>>, ~scrollTimestamp: float) => {
  ...
  | Some(info) => {
      ...
      <>
        <div
          className="absolute bg-fm-accent/[0.08] border-[1.5px] border-fm-accent/70 rounded-sm pointer-events-none z-[9998] box-border transition-all duration-100 ease-out"
          style={
