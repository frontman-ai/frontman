@val
external nodeListToElements: WebAPI.DomTypes.nodeList<WebAPI.DomTypes.element> => array<
  WebAPI.DomTypes.element,
> = "Array.from"
@get external innerText: WebAPI.DomTypes.htmlElement => string = "innerText"
...
let getChildElements = (el: WebAPI.DomTypes.element, ~pierceShadowDom: bool): array<
  WebAPI.DomTypes.element,
> => {
  let children = el.children
  let result: array<WebAPI.DomTypes.element> = []
  ...
  | true =>
    ...
    | Some(shadowRoot) =>
      ...
      for i in 0 to childNodes.length - 1 {
        let node = WebAPI.NodeList.item(childNodes, i)
        switch WebAPI.Node.nodeType(node) === 1 {
        | true => result->Array.push(Obj.magic(node))->ignore
        | false => ()
        }
      }
    | None => ()
...
let hasShadowRoot = (el: WebAPI.DomTypes.element): bool =>
  el.shadowRoot->Null.toOption->Option.isSome

let effectiveRole = (el: WebAPI.DomTypes.element): string => {
  let rawRole =
...
let getOptionalRole = (el: WebAPI.DomTypes.element): option<string> =>
  switch FrontmanBindings.Bindings__DomAccessibilityApi.getRole(el)->Null.toOption {
  | Some("") | None => None
  | some => some
  }

let getOptionalAccessibleName = (el: WebAPI.DomTypes.element): option<string> =>
...
let getCursor = (win: WebAPI.DomTypes.window, el: WebAPI.DomTypes.element): string =>
  try {
...
let hasZeroDimensions = (el: WebAPI.DomTypes.element): bool => {
  let rect = el->WebAPI.Element.getBoundingClientRect
  rect.width <= 0.0 || rect.height <= 0.0
}

let isEffectivelyHidden = (el: WebAPI.DomTypes.element): bool =>
...
let getVisibleText = (el: WebAPI.DomTypes.element): string =>
  try {
    let htmlEl = el->Obj.magic
    switch innerText(htmlEl) {
    | "" => (el :> WebAPI.DomTypes.node)->WebAPI.Node.textContent->Null.toOption->Option.getOr("")
    | text => text
    }
  } catch {
  | JsExn(_) => ""
...
let childMatchesText = (el: WebAPI.DomTypes.element, lowerText: string): bool => {
  let children = el.children
  let found = ref(false)
  let j = ref(0)
...
let resolveByText = (~document: WebAPI.DomTypes.document, ~text: string, ~index: int): (
  option<WebAPI.DomTypes.element>,
  int,
) => {
  let bodyEl = document.body->Obj.magic
...
let generateSelector = (
  ...
  try {
    ...
    | Some(doc) => doc.documentElement->Obj.magic
    | None => element
    }
...
let describeElement = (el: WebAPI.DomTypes.element): string => {
