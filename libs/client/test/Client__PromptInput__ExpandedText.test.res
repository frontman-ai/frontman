...
let _getTextContentOrThrow = value => value->WebAPI.Node.textContent->Null.toOption->Option.getOrThrow

let _appendChildren = (parent: WebAPI.DOMAPI.element, children: array<WebAPI.DOMAPI.node>) => {
  children->Array.forEach(child => parent->WebAPI.Element.asNode->WebAPI.Node.appendChild(child)->ignore)
  parent
}
...
describe("getExpandedTextFromEditable", () => {
  describe("happy path", () => {
    test("returns plain text when there are no chips", t => {
    ...
    test("expands a single pasted-text chip inline", t => {
      let el = editable([
        text("Look at this: "),
        pasteChip("p1"),
    ...
    test("preserves ordering with chip between two text segments", t => {
    ...
    test("handles multiple pasted-text chips in correct order", t => {
      let el = editable([
        text("A "),
        pasteChip("p1"),
        text(" B "),
    ...
    test("handles chip at the very start", t => {
    ...
    test("handles chip at the very end", t => {
    ...
    test("handles only a pasted-text chip with no typed text", t => {
    ...
    test("handles consecutive chips with no text between them", t => {
  ...
  describe("file chip handling", () => {
    test("skips file attachment chips entirely", t => {
    ...
    test("expands paste chips but skips file chips in mixed content", t => {
      let el = editable([
        text("A "),
        fileChip("f1"),
        text(" B "),
  ...
  describe("edge cases", () => {
    ...
    test("handles DIV-wrapped lines (browser contentEditable behavior)", t => {
    ...
    test("handles paste chip inside a DIV wrapper", t => {
    ...
    test("handles chip with missing map entry (not in map) — treated as skipped", t => {
    ...
    test("handles multiline pasted content preserving internal newlines", t => {
    ...
    test("handles whitespace-only typed text around chips", t => {
    ...
    test("handles deeply nested elements (P > span > text)", t => {
