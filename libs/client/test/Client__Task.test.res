...
describe("Task - QuestionReceived on freshly loaded task (reconnect scenario)", () => {
  ...
  test("replayed question calls share the existing answer", t => {
    let task = TestHelpers.makeLoadedTask()
    let questions: array<Client__Question__Types.questionItem> = [
      {
        question: "Pick one",
    ...
    let firstAnswer = ref(None)
    let replayAnswer = ref(None)
    let (pending, _) = TaskReducer.next(
      task,
      QuestionReceived({
        questions,
        toolCallId: "tc_1",
        resolveOk: answer => firstAnswer := Some(answer),
        resolveError: _ => (),
      }),
    )
    let (replayed, replayEffects) = TaskReducer.next(
      pending,
      QuestionReceived({
        questions,
        toolCallId: "tc_1",
        resolveOk: answer => replayAnswer := Some(answer),
        resolveError: _ => (),
      }),
    )
    let (answered, effects) = TaskReducer.next(replayed, QuestionAllSkipped)

    t->expect(replayEffects->Array.length)->Expect.toBe(0)
    ...
    t->expect(Option.isSome(firstAnswer.contents))->Expect.toBe(true)
    t->expect(replayAnswer.contents)->Expect.toEqual(firstAnswer.contents)
    t->expect(TaskReducer.Selectors.pendingQuestion(answered))->Expect.toBe(None)
  })

  test("a different question cannot replace an active call", t => {
    let task = TestHelpers.makeLoadedTask()
    let questions: array<Client__Question__Types.questionItem> = [
      {
        question: "Pick one",
    ...
    let (unchanged, effects) = TaskReducer.next(
      ...
      QuestionReceived({
        ...
        toolCallId: "tc_2",
        resolveOk: _ => (),
        resolveError: error => conflictingError := Some(error),
      }),
    )
    ...
    t
    ->expect(
      TaskReducer.Selectors.pendingQuestion(unchanged)->Option.map(question => question.toolCallId),
    )
    ->Expect.toBe(Some("tc_1"))
    t->expect(conflictingError.contents)->Expect.toBe(Some("Another question is already pending"))
  })

  test("a changed replay payload cannot attach to an active call", t => {
    let task = TestHelpers.makeLoadedTask()
    let firstQuestions: array<Client__Question__Types.questionItem> = [
      {
    ...
    let changedQuestions: array<Client__Question__Types.questionItem> = [
      {
    ...
    let (_, effects) = TaskReducer.next(
      ...
      QuestionReceived({
        ...
        toolCallId: "tc_1",
        resolveOk: _ => (),
        resolveError: error => replayError := Some(error),
      }),
    )
    ...
    t->expect(replayError.contents)->Expect.toBe(Some("Another question is already pending"))
  })

  test("connection loss rejects every waiter and clears the question", t => {
    let task = TestHelpers.makeLoadedTask()
    let questions: array<Client__Question__Types.questionItem> = [
      {
        question: "Pick one",
    ...
    let (replayed, _) = TaskReducer.next(
      pending,
      QuestionReceived({
        questions,
        toolCallId: "tc_1",
    ...
    let (disconnected, effects) = TaskReducer.next(
      replayed,
      QuestionTerminated({message: "Connection lost"}),
    )
    ...
    t->expect(errors.contents)->Expect.toEqual(["Connection lost", "Connection lost"])
    t->expect(TaskReducer.Selectors.pendingQuestion(disconnected))->Expect.toBe(None)
  })

  test("cancel and agent error settle a pending reconnect question", t => {
    [
      (TaskReducer.CancelTurn, "Cancelled by user"),
      (
        TaskReducer.AgentError({id: "err-1", error: "Agent failed", category: #unknown}),
        "Agent failed",
      ),
    ]->Array.forEach(
      ((action, expectedError)) => {
        ...
        let (settled, effects) = TaskReducer.next(pending, action)
        ...
        t->expect(TaskReducer.Selectors.pendingQuestion(settled))->Expect.toBe(None)
      },
    )
  })
