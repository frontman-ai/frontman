...
The plugin uses the current MCP Streamable HTTP protocol at `/mcp`, validates the site Origin, requires the authenticated administrator session and a WordPress nonce, validates and sanitizes tool inputs, and restricts option changes to a safe allowlist. Destructive edits require careful review, and you should keep backups before using any AI editing tool on an important site.
